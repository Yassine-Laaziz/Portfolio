...
// Galaxy component
function Galaxy({ position, scale = 1, color = '#6366f1' }: {
  ...
  useFrame((state) => {
    if (galaxyRef.current) {
      ...
      galaxyRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.001) * 0.001
    }
  })
...
function ShootingStar({ startPosition, endPosition, delay = 0 }: {
  ...
  useFrame((state) => {
    if (starRef.current && trailRef.current) {
      ...
      if (elapsed > 0) {
        ...
        if (progress < 0.8) { // Visible for 80% of cycle
          ...
          if (trailRef.current.material && 'opacity' in trailRef.current.material) {
...
function CosmicScene() {
  return (
    <>
      ...
      <Galaxy position={[25, -5, -40]} scale={3} color="#8b5cf6" />
      <Galaxy position={[-5, -2, -5]} scale={1} color="#06b6d4" />
